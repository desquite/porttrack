...
import { INFRACTION_STATUTS } from "@porttrack/shared";
...
export function InfractionsFilters() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      params.delete("page");
      startTransition(() => router.replace(`/infractions?${params.toString()}`));
    }, 300);
  ...
  function setStatut(value: string) {
    ...
    params.delete("page");
    router.replace(`/infractions?${params.toString()}`);
  }
  function reset() { setQ(""); router.replace("/infractions"); }
  const hasFilters = q || statut;

  return (
    <div className="flex flex-wrap items-end gap-3 rounded-md border bg-muted/30 p-3">
      <div className="flex-1 min-w-[200px]">
        <Label htmlFor="inf-q" className="text-xs">Recherche</Label>
        <div className="relative">
          <Search className="absolute left-2.5 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" />
          <Input id="inf-q" type="search" value={q} onChange={(e) => setQ(e.target.value)}
            placeholder="Type, lieu, description…" className="pl-8" />
        </div>
      </div>
      <div>
        <Label htmlFor="inf-statut" className="text-xs">Paiement</Label>
        <select id="inf-statut" value={statut} onChange={(e) => setStatut(e.target.value)}
